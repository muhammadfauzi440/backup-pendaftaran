...
<body class="bg-gray-50 font-['Plus_Jakarta_Sans'] text-gray-900 overflow-x-hidden" x-data="{ sidebarOpen: window.innerWidth >= 768 }">

    <div x-show="sidebarOpen" x-transition.opacity @click="sidebarOpen = false"
        class="fixed inset-0 z-50 bg-gray-900/50 backdrop-blur-sm md:hidden" aria-hidden="true" style="display: none;">
    </div>
    ...
    <aside :class="sidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full md:translate-x-0 md:w-20'"
        ...
        <nav class="mt-8 px-4 space-y-2 flex flex-col h-[calc(100vh-120px)] overflow-y-auto overflow-x-hidden">
            ...
            <a href="{{ route('admin.pendaftaran.index') }}"
                ...
                <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('admin.pendaftaran.*') ? 'text-white' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.instansi.index') }}"
                ...
                <span x-show="sidebarOpen" class="font-bold whitespace-nowrap">Data Instansi</span>
            </a>
            ...
            <form action="{{ route('logout') }}" method="POST" class="pb-6 mt-auto">
                ...
                <button type="submit"
                    ...
                    <span x-show="sidebarOpen" class="font-black text-xs uppercase tracking-[0.2em] whitespace-nowrap">Logout</span>
                </button>
            </form>
        </nav>
    </aside>

    <div :class="sidebarOpen ? 'md:ml-64' : 'md:ml-20'" class="flex flex-col min-h-screen transition-all duration-300">
        
        <header class="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-4 md:px-8 sticky top-0 z-40">
            <button @click="sidebarOpen = !sidebarOpen" class="p-2 hover:bg-gray-50 rounded-lg text-gray-500 transition-colors">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path>
                </svg>
            </button>
            
            <span class="md:hidden font-bold text-gray-900">Admin Panel</span>
            <div class="w-8 md:hidden"></div> 
        </header>

        <div class="p-4 pb-6 overflow-y-auto overflow-x-hidden relative z-0 w-full max-w-full" style="height: calc(100vh - 80px);">
            @hasSection('content')
                @yield('content')
            @else
                ...
                <div class="mb-6 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                    <div>
                        ...
                        <p class="text-gray-500 text-sm font-medium mt-1">Ringkasan analitik pendaftaran magang PT Global Intermedia.</p>
                    </div>
                    <div class="flex items-center gap-3 bg-white p-2 rounded-xl border border-gray-200 shadow-sm">
                        <label for="tahunFilter" class="text-xs font-bold text-gray-500 uppercase tracking-wider pl-2">Tahun:</label>
                        <select id="tahunFilter" onchange="filterTahun(this.value)" class="px-4 py-2 border-none rounded-lg bg-gray-50 text-sm font-bold text-gray-900 focus:ring-0 cursor-pointer outline-none hover:bg-gray-100 transition-colors">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2 bg-white rounded-3xl border border-gray-100 shadow-sm p-6 min-w-0 overflow-hidden">
                        <h3 class="text-sm font-black text-gray-900 uppercase tracking-wider mb-2">Statistik Bulanan {{ $selectedYear }}</h3>
                        <p class="text-xs text-gray-500 font-medium mb-4">Statistik pendaftar masuk dari bulan Januari hingga Desember.</p>
