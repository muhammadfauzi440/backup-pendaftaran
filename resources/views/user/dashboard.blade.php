...
<head>
    ...
    @vite(['resources/css/app.css', 'resources/js/app.js', 'resources/js/daftar.js', 'resources/js/kelompok.js', 'resources/js/copy.js'])
...
<body class="bg-[#FDFDFD] font-['Plus_Jakarta_Sans'] overflow-x-hidden" x-data="{ sidebarOpen: window.innerWidth >= 768, showNotifModal: false }">
    ...
    <div class="flex min-h-screen w-full">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full md:translate-x-0 md:w-20'"
            ...
            <div class="p-6 flex items-center justify-between border-b border-gray-800 h-20">
                ...
                <span x-show="!sidebarOpen"
                    class="font-black text-xl tracking-tight hidden md:block w-full text-center text-red-600">GI</span>
            ...
            <nav class="mt-8 px-4 space-y-2 flex flex-col h-[calc(100vh-120px)] overflow-y-auto overflow-x-hidden">
                ...
                <form action="{{ route('logout') }}" method="POST" class="pt-4 mt-auto pb-6">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center gap-4 px-4 py-4 bg-red-600 hover:bg-red-700 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl shadow-lg shadow-red-900/20 transition-all transform hover:-translate-y-1 group"
                        title="Keluar Akun">
                        <svg class="w-4 h-4 shrink-0 group-hover:scale-110 transition-transform" fill="none"
        ...
        <div class="flex-1 flex flex-col transition-all duration-300 w-full max-w-full"
            :class="sidebarOpen ? 'md:ml-64' : 'md:ml-20'">

            <header
                class="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-4 sm:px-8 sticky top-0 z-40 shadow-sm md:shadow-none">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div
                        class="w-10 h-10 bg-red-100 text-red-600 rounded-full flex items-center justify-center font-black border-2 border-white shadow-sm uppercase">
            ...
            <main class="p-4 sm:p-12 overflow-x-hidden relative z-0">
                ...
                @else
                    <div class="max-w-5xl mx-auto">
                        <h1
                            class="text-3xl sm:text-4xl font-black text-gray-900 mb-2 uppercase tracking-tighter italic">
                            Halo,
                            {{ auth()->user()->name }}! 👋</h1>
                        <p class="text-gray-500 mb-10 text-sm sm:text-lg">Selamat datang di portal magang PT Global
                            Intermedia
                            Nusantara.</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-5 gap-8">
                            ...
                            <div
                                class="md:col-span-3 bg-white border-2 border-gray-50 p-6 sm:p-8 rounded-3xl sm:rounded-[2.5rem] shadow-sm overflow-hidden">
                                ...
                                @if (isset($pendaftaran) && $pendaftaran)
                                    ...
                                    @if ($pendaftaran->status == 'diterima')
                                        <div
                                            class="mt-2 mb-6 p-4 bg-emerald-50 border border-emerald-200 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-sm">
                                            <div>
                                                <h3
                                                    class="text-emerald-800 font-black text-sm uppercase tracking-widest">
                                                    Selamat! Anda Diterima</h3>
                                                <p class="text-emerald-600 text-[10px] font-bold mt-1">Unduh Surat
                                                    Balasan/LoA untuk diserahkan ke instansi Anda.</p>
                                            </div>
                                            <a href="{{ route('user.cetak-surat') }}"
                                                class="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-600/30 transition transform hover:-translate-y-1 flex items-center justify-center gap-2">
                                    ...
                                    <div
                                        class="mb-6 p-4 sm:p-5 bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl flex justify-between items-center group hover:border-red-300 transition-colors">
                                        <div class="min-w-0 pr-2">
                                            <span
                                                class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Kode
                                                Pendaftaran</span>
                                            <span
                                                class="text-xl sm:text-2xl font-black text-gray-900 tracking-widest truncate block"
                                                id="kode-pendaftaran">{{ $pendaftaran->kode_pendaftaran ?? '-' }}</span>
                                        </div>
                                    ...
                                    <div class="space-y-4 mt-2">
                                        ...
                                        <div
                                            class="flex flex-col sm:flex-row sm:justify-between border-b border-gray-50 pb-3 sm:items-end gap-1">
                                            <span class="text-gray-400 text-xs font-bold uppercase">Tipe
                                                Pendaftaran</span>
                                            <span
                                                class="text-gray-900 font-black text-sm uppercase">{{ $pendaftaran->tipe_pendaftaran ?? '-' }}</span>
                                        </div>
                                        <div
                                            class="flex flex-col sm:flex-row sm:justify-between border-b border-gray-50 pb-3 sm:items-end gap-1">
                                            <span class="text-gray-400 text-xs font-bold uppercase">Instansi /
                                                Kampus</span>
                                            <span
                                                class="text-gray-900 font-black text-sm text-left sm:text-right">{{ $pendaftaran->nama_instansi_display }}</span>
                                        </div>
                                        <div
                                            class="flex flex-col sm:flex-row sm:justify-between border-b border-gray-50 pb-3 sm:items-center gap-2">
                                            <span class="text-gray-400 text-xs font-bold uppercase">Periode
                                                Magang</span>
                                            <div class="text-left sm:text-right">
                                                ...
                                                <span
                                                    class="bg-gray-100 text-gray-500 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider inline-block mt-1">
                                        ...
                                        <div class="pt-6">
                                            @can('update', $pendaftaran)
                                                <a href="/user/daftar"
                                                    class="block text-center sm:inline-block bg-red-600 text-white px-8 py-4 rounded-2xl text-xs font-black uppercase tracking-[0.2em] hover:bg-red-700 transition-all shadow-xl shadow-red-600/20 transform hover:-translate-y-1">
                                                    Edit / Lihat Formulir &rarr;
                                                </a>
                                            @else
                                                <button disabled
                                                    class="w-full sm:w-auto bg-gray-200 text-gray-500 cursor-not-allowed px-8 py-4 rounded-2xl text-xs font-black uppercase tracking-[0.2rem] shadow-sm">
                                                    Formulir Terkunci (Diproses/Selesai)
                                                </button>
                                                <p class="text-xs font-bold text-gray-400 mt-3 text-center sm:text-left">
                                                    *Anda tidak dapat lagi mengubah data pendaftaran karena status sudah
                                                    bukan "Pending"
                                                </p>
                                            @endcan
                                        </div>
                                ...
                                @else
                                    <div class="py-10 text-center">
                                        ...
                                        <p class="text-gray-400 mb-8 font-medium text-sm">Anda belum mengisi formulir
                            ...
                            <div
                                class="md:col-span-2 bg-gray-900 p-6 sm:p-8 rounded-3xl sm:rounded-[2.5rem] shadow-2xl text-white flex flex-col justify-between relative overflow-hidden group">
                                <div
                                ...
                                <div class="relative z-10">
                                    <div
                                        class="w-16 h-16 bg-red-600 rounded-2xl flex items-center justify-center mb-6 shadow-lg shadow-red-600/40">
                                    ...
                                    <h2 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Pusat
                                        Bantuan</h2>
