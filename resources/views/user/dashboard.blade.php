    ...
    <div class="flex min-h-screen">
        ...
        <aside :class="sidebarOpen ? 'w-64' : 'w-20'"
            ...
            <nav class="mt-8 px-4 space-y-2 flex flex-col h-[calc(100vh-120px)]">
                ...
                <form action="{{ route('logout') }}" method="POST" class="pt-4">
                    @csrf
                    <button type="submit" class="w-full flex items-center gap-4 px-4 py-4 bg-red-600 hover:bg-red-700 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl shadow-lg shadow-red-900/20 transition-all transform hover:-translate-y-1">
        ...
        <div class="flex-1 flex flex-col transition-all duration-300" :class="sidebarOpen ? 'ml-64' : 'ml-20'">
            <header class="h-20 bg-white border-b border-gray-100 flex items-center px-8 sticky top-0 z-40">
                <button @click="sidebarOpen = !sidebarOpen" class="p-2 hover:bg-gray-50 rounded-lg text-gray-500 transition">
            ...
            <main class="p-12">
                ...
                @else
                    <div class="max-w-5xl mx-auto">
                        <h1 class="text-4xl font-black text-gray-900 mb-2 uppercase tracking-tighter italic">Halo, {{ auth()->user()->name }}! 👋</h1>
                        <p class="text-gray-500 mb-10 text-lg">Selamat datang di portal magang PT Global Intermedia Nusantara.</p>

                        <div class="grid grid-cols-1 md:grid-cols-5 gap-8">
                            
                            <div class="md:col-span-3 bg-white border-2 border-gray-50 p-8 rounded-[2.5rem] shadow-sm">
                                <h2 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-6">Status Pendaftaran Anda</h2>
                                
                                @if (isset($pendaftaran) && $pendaftaran)
                                    <div class="inline-flex items-center px-6 py-2 rounded-full mb-6 font-black uppercase tracking-widest text-[10px]
                                    ...
                                    <div class="space-y-4 mt-2">
                                        ...
                                        <div class="flex justify-between border-b border-gray-50 pb-3 items-end">
                                            ...
                                            <span class="text-gray-900 font-black text-sm">{{ $pendaftaran->instansi->nama_instansi ?? '-' }}</span>
                                        </div>
                                        <div class="flex justify-between border-b border-gray-50 pb-3 items-center">
                                            ...
                                            <div class="text-right">
                                                <span class="text-gray-900 font-black text-sm block mb-1">
                                                    {{ \Carbon\Carbon::parse($pendaftaran->tanggal_mulai)->format('d M Y') }} - {{ \Carbon\Carbon::parse($pendaftaran->tanggal_selesai)->format('d M Y') }}
                                                </span>
                                                <span class="bg-gray-100 text-gray-500 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider">
                            ...
                            <div class="md:col-span-2 bg-gray-900 p-8 rounded-[2.5rem] shadow-2xl text-white flex flex-col justify-between relative overflow-hidden group">
                                <div class="absolute -right-6 -top-6 opacity-5 group-hover:opacity-10 transition-opacity duration-500 transform rotate-12">
                                    <svg class="w-48 h-48" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0a12 12 0 00-3.537 23.472l3.666-1.155A11.95 11.95 0 0112 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.823a9.78 9.78 0 01-4.992-1.353l-.358-.212-2.484.783.664-2.424-.233-.37A9.78 9.78 0 012.18 12c0-5.414 4.406-9.82 9.82-9.82 5.414 0 9.82 4.406 9.82 9.82 0 5.414-4.406 9.82-9.82 9.82z"/></svg>
                                </div>
